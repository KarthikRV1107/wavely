...
const MOODS = [
  ...
  { label:'Sleep', query:'sleep ambient music', color:'#a78bfa', bg:'rgba(167,139,250,0.1)' },
  { label:'Happy', query:'happy feel good pop', color:'#fbbf24', bg:'rgba(251,191,36,0.1)' },
  { label:'Hip-Hop', query:'hip hop beats 2024', color:'#34d399', bg:'rgba(52,211,153,0.1)' },
  { label:'Rock', query:'rock anthems hits', color:'#fb923c', bg:'rgba(251,146,60,0.1)' },
];
...
export default function SearchPage() {
  ...
  const handleMood = (m) => { setActive(m.label); setQuery(m.query); };
    ...
    <div style={{ maxWidth:680, margin:'0 auto', padding:'0 0 8px' }}>
      ...
      <div style={{
        ...
        <div style={{
          ...
          <input ref={inputRef} type="search" placeholder="Songs, artists, albums…"
            value={query} onChange={e => { setQuery(e.target.value); setActive(null); }}
            style={{
          ...
          {query && !loading && (
            <button onClick={() => { setQuery(''); setResults([]); setActive(null); }}
              style={{ background:'none', border:'none', color:'var(--text3)',
                       fontSize:18, lineHeight:1, cursor:'pointer', padding:0 }}>×</button>
          )}
        </div>
      </div>
