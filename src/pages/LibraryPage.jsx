...
export default function LibraryPage() {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      const pl = { id, name: name.trim(), songCount: 0 };
      setPlaylists(p => [pl, ...p]);
  ...
  return (
    <div style={{ maxWidth:680, margin:'0 auto', padding:'0 0 8px' }}>
      <div style={{ padding:'20px 20px 0' }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:20 }}>
          <h1 style={{ fontFamily:'var(--font-display)', fontSize:26, fontWeight:800,
                       letterSpacing:'-0.02em', color:'var(--text1)', margin:0 }}>Library</h1>
          <button onClick={() => setModal(true)} style={{
            display:'flex', alignItems:'center', gap:6,
            padding:'8px 16px', background:'linear-gradient(135deg, var(--accent), #5b4fcf)',
            border:'none', borderRadius:20, fontSize:13, fontWeight:600, color:'#fff',
            cursor:'pointer', boxShadow:'0 4px 16px rgba(124,106,247,0.35)',
          }}>
      ...
      <div style={{ padding:'0 20px' }}>
        ...
        {!loading && tab === 1 && (
          liked.length === 0
            ? <Empty icon="❤️" text="No liked songs yet" sub="Tap the heart on any song"/>
            : <div style={{ display:'flex', flexDirection:'column', gap:4 }}>
                {liked.map((song, i) => (
                  <SongCard key={song.id} song={song}
                    queue={liked} queueIndex={i} isLiked={true}
      ...
      {modal && (
        <div style={{
          ...
          <div style={{
            ...
            border:'1px solid var(--border2)', padding:28, width:'100%', maxWidth:360,
