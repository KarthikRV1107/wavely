// src/components/Layout.jsx  — fully responsive: sidebar on desktop, bottom nav on mobile
...
const NAV = [
  {
    ...
    icon: (a) => (
      <svg width="20" height="20" viewBox="0 0 24 24" fill="none"
        stroke={a ? 'var(--accent2)' : 'var(--text3)'} strokeWidth="1.8">
...
const isActive = (path, exact, pathname) =>
    ...
    <aside className="app-sidebar" style={{ padding: '0' }}>
      ...
      <nav style={{ padding: '12px 12px', flex: 1 }}>
        ...
        {NAV.map(({ path, label, exact, icon }) => {
          const active = isActive(path, exact, pathname);
          return (
            <button key={path} onClick={() => navigate(path)} style={{
              ...
              borderRadius: 12, textAlign: 'left', transition: 'all 0.18s',
              cursor: 'pointer',
            }}
...
export default function Layout({ children }) {
  ...
  // Extra bottom padding so content isn't hidden behind player + nav
  const bottomPad = currentSong
    ? isDesktop
      ? 'calc(var(--player-h) + 24px)'
  ...
  return (
    <div className="app-shell">
      ...
      <main className="app-main" style={{
        ...
        minHeight: '100vh',
        transition: 'padding-bottom 0.3s var(--ease-out)',
      }}>
        {/* Responsive breadcrumb indicator (desktop only) */}
        {isDesktop && (
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              {NAV.find(n => isActive(n.path, n.exact, pathname)) && (
                <span style={{ fontSize: 13, color: 'var(--text3)' }}>
                  {NAV.find(n => isActive(n.path, n.exact, pathname))?.label}
                </span>
              )}
            </div>
